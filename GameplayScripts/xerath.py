...
def combo(game):
		...
		if use_q_in_combo and IsReady(game, q_spell):
			target = game.GetBestTarget(UnitTag.Unit_Champion,max_q_range)
			if target:
				if not charging_q:
		...
		if use_e_in_combo and IsReady(game, e_spell) :
			target = game.GetBestTarget(UnitTag.Unit_Champion,1050)
			if target:
		...
		if use_w_in_combo and IsReady(game, w_spell) :
			target = game.GetBestTarget(UnitTag.Unit_Champion,1100)
			if target:
...
def steal_with_r(game):
		...
		if disable_ks_key and IsReady(game, r_spell) and not charging_q:
			target = game.GetBestTarget(UnitTag.Unit_Champion,5)
			if target:
